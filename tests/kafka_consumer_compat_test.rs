...
fn kafka_compat_offsets_strictly_increasing_per_partition() {
    ...
    assert_eq!(
        offsets,
        [0, 1, 2, 3, 4],
        "fetched offsets must be strictly increasing"
    );
}
...
fn kafka_compat_consumer_group_resume_after_commit_no_duplicates() {
    ...
    let producer = Producer::new(Arc::clone(&broker));
    producer
        .send("events", Record::new(b"a".to_vec()), None)
        .unwrap();
    producer
        .send("events", Record::new(b"b".to_vec()), None)
        .unwrap();
    producer
        .send("events", Record::new(b"c".to_vec()), None)
        .unwrap();
    ...
    assert_eq!(
        batch2.len(),
        0,
        "resumed consumer must not see already-committed records"
    );
}
...
fn kafka_compat_subscribe_multiple_partitions_poll_returns_all() {
    ...
    let producer = Producer::new(Arc::clone(&broker));
    producer
        .send("multi", Record::new(b"p0-a".to_vec()), Some(0))
        .unwrap();
    producer
        .send("multi", Record::new(b"p0-b".to_vec()), Some(0))
        .unwrap();
    producer
        .send("multi", Record::new(b"p1-a".to_vec()), Some(1))
        .unwrap();
    ...
    let by_partition: std::collections::HashMap<i32, Vec<_>> = batch
        .iter()
        .map(|r| (r.partition, r.offset))
        .fold(std::collections::HashMap::new(), |mut m, (p, o)| {
            m.entry(p).or_default().push(o);
            m
        });
    assert_eq!(
        by_partition.get(&0).map(|v| v.as_slice()),
        Some([0, 1].as_slice())
    );
    assert_eq!(
        by_partition.get(&1).map(|v| v.as_slice()),
        Some([0].as_slice())
    );
}
...
fn kafka_compat_assign_single_partition_only_sees_that_partition() {
    ...
    let producer = Producer::new(Arc::clone(&broker));
    producer
        .send("t", Record::new(b"p0".to_vec()), Some(0))
        .unwrap();
    producer
        .send("t", Record::new(b"p1".to_vec()), Some(1))
        .unwrap();
...
fn kafka_compat_seek_to_beginning() {
    ...
    broker.create_topic("t", None).unwrap();
    broker
        .produce("t", Some(0), Record::new(b"first".to_vec()))
        .unwrap();
    broker
        .produce("t", Some(0), Record::new(b"second".to_vec()))
        .unwrap();
...
fn kafka_compat_poll_empty_after_consuming_all() {
    ...
    broker.create_topic("t", None).unwrap();
    broker
        .produce("t", Some(0), Record::new(b"only".to_vec()))
        .unwrap();
    let mut consumer = Consumer::new(Arc::clone(&broker));
    consumer.subscribe(vec!["t".to_string()]).unwrap();
...
fn kafka_compat_high_water_mark_is_next_assignable_offset() {
    ...
    broker
        .produce("t", Some(0), Record::new(b"a".to_vec()))
        .unwrap();
    assert_eq!(broker.high_water_mark("t", 0).unwrap(), 1);
    broker
        .produce("t", Some(0), Record::new(b"b".to_vec()))
        .unwrap();
...
fn kafka_compat_stored_record_has_partition_and_offset() {
    ...
    broker
        ...
        .unwrap();
    broker
        .produce("t", Some(1), Record::new(b"v".to_vec()))
        .unwrap();
